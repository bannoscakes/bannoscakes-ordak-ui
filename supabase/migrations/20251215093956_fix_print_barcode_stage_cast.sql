...
-- Date: 2025-12-15
--
-- Issue: v_order.stage is stage_type enum, but stage_events.stage is text
-- Fix: Cast v_order.stage::text when inserting into stage_events
-- ============================================================================
...
BEGIN
  ...
  EXECUTE format(
    'SELECT id, shopify_order_number, product_title, size, due_date, customer_name, stage::text, priority, filling_start_ts FROM %I WHERE id = $1',
    v_table_name
  ...
  -- Log print event to stage_events (stage is already text from cast above)
  INSERT INTO public.stage_events (
    ...
    p_store,
    p_order_id,
    v_order.stage,  -- Already cast to text in SELECT
    'print',
    now(),
    v_user_id,
