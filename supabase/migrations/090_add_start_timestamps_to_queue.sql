-- Migration 090: Add covering_start_ts and decorating_start_ts to get_queue RPC
-- Purpose: Scanner needs these timestamps to determine if stage was started or should be completed
-- These columns were added in migration 089, now we expose them via get_queue

BEGIN;

-- Drop and recreate get_queue with new columns
...
COMMENT ON FUNCTION public.get_queue IS 'Get queue of orders with optional filtering. Now includes covering_start_ts and decorating_start_ts for scanner stage logic.';
