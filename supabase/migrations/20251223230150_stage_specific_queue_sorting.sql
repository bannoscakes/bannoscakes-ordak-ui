-- Migration: Add stage-specific sorting to get_queue RPC
--
-- Sorting rules by stage (based on order's ACTUAL stage, not filter):
-- - Filling: Priority -> Flavour -> Order Number
-- - Covering & Decorating: Priority -> Product Title -> Size
-- - Packing: Priority -> Size
-- - Complete/Default: Priority -> Order Number

BEGIN;

-- Drop and recreate get_queue with stage-specific sorting
...
COMMENT ON FUNCTION public.get_queue IS 'Get queue of orders with stage-specific sorting. Filling: Priority->Flavour->Order#. Covering/Decorating: Priority->Product->Size. Packing: Priority->Size.';
