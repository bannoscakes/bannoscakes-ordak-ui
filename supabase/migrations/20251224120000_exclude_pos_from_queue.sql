-- Migration: Exclude POS orders from get_queue RPC
--
-- POS orders are point-of-sale transactions (already complete).
-- They should not appear in the production queue.
-- Filter: Exclude orders where source_name = 'pos'

BEGIN;

-- Drop and recreate get_queue with POS exclusion filter
...
COMMENT ON FUNCTION public.get_queue IS 'Get queue of orders with stage-specific sorting. Excludes POS orders (source_name=pos). Filling: Priority->Flavour->Order#. Covering/Decorating: Priority->Product->Size. Packing: Priority->Size.';
