-- Fix accessory matching to include variant_title
--
-- Problem: Order arrives with:
--   title = "Pink Glitter Number Candles"
--   variant_title = "Pink Glitter / 6"
-- But inventory item is: "Pink Glitter Number Candles 6"
--
-- The trigger only matched on title, ignoring variant_title.
--
-- Solution: Combine title + variant suffix for matching:
--   1. Extract value after "/" from variant_title (e.g., "Pink Glitter / 6" → "6")
--   2. If no "/", use entire variant_title (e.g., "Silver" → "Silver")
--   3. Build combined string: title + " " + variant_value
--   4. Match against product_match
...
BEGIN
  ...
  -- [{"title": "Pink Glitter Number Candles", "variant_title": "Pink Glitter / 6", "quantity": 1}, ...]
  ...
  IF NEW.accessories IS NOT NULL
    ...
    LOOP
      ...
      -- If variant_title exists, extract the value after "/" (or use whole string if no "/")
      IF v_variant_title IS NOT NULL THEN
        -- Check if variant_title contains "/"
...
COMMENT ON FUNCTION public.deduct_inventory_on_order IS 'Trigger function to deduct inventory when orders are inserted. Matches accessories by keyword using combined title+variant_title, cake_toppers by exact product_title. Never fails the order insert.';
