...
BEGIN
  ...
  -- Add staff_shared to publication if not already added
  IF NOT EXISTS (
    ...
    ALTER PUBLICATION supabase_realtime ADD TABLE public.staff_shared;
  END IF;

  -- Add conversations to publication if not already added
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
    AND schemaname = 'public' 
  ...
  -- Add messages to publication if not already added
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
    AND schemaname = 'public' 
  ...
  -- Add conversation_participants to publication if not already added
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
    AND schemaname = 'public' 
