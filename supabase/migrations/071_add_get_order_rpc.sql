-- Migration: Add get_order RPC
-- Purpose: Fetch a single order by ID with all fields needed for OrderDetailDrawer
-- This RPC was missing, causing the drawer to fall back to incomplete data

BEGIN;

-- Create get_order function to fetch a single order by ID
...
-- Add comment
COMMENT ON FUNCTION public.get_order IS 'Fetch a single order by ID with all fields needed for order detail view. Returns cake_writing, product_image, delivery_method correctly.';
