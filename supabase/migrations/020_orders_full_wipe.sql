...
-- Schema-qualified to avoid search_path issues.

DO $$
DECLARE
  suffix text := to_char(clock_timestamp(), 'YYYYMMDD_HH24MISSUS');  -- microseconds
  tbl text;
...
BEGIN
  -- Backups (only if table exists). Always create backups in public schema.
  FOREACH tbl IN ARRAY dep_tables || order_tables LOOP
    IF to_regclass('public.'||tbl) IS NOT NULL THEN
      EXECUTE 'CREATE TABLE '
             || quote_ident('public') || '.' || quote_ident(tbl || '_bak_' || suffix)
             || ' AS TABLE '
             || quote_ident('public') || '.' || quote_ident(tbl);
    END IF;
  END LOOP;

  -- Truncate dependents first (public schema explicitly)
