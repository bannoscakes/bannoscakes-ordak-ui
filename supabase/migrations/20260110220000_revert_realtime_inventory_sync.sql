-- ============================================================================
-- Migration: Revert Real-time Inventory Sync Changes
-- Purpose: Undo migrations 20260110205859, 20260110210500, 20260110211000
-- ============================================================================

BEGIN;

-- ============================================================================
-- STEP 1: Drop the trigger_attempted_at column added in migration 211000
-- ============================================================================

ALTER TABLE public.inventory_sync_queue
DROP COLUMN IF EXISTS trigger_attempted_at;

-- ============================================================================
-- STEP 2: Drop the idx_accessories_sku index added in migration 210500
-- ============================================================================

DROP INDEX IF EXISTS public.idx_accessories_sku;

-- ============================================================================
-- STEP 3: Remove the supabase_project_url setting if it exists
-- ============================================================================
...
-- ============================================================================
-- STEP 4: Restore the cron job that was removed in migration 205859
-- ============================================================================

DO $outer$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- Remove if exists first (idempotent)
    ...
    -- Re-create the 5-minute cron job
    ...
    RAISE NOTICE 'Restored sync-inventory-to-shopify cron job';
  END IF;
END;
$outer$;

-- ============================================================================
-- STEP 5: Restore original deduct_inventory_on_order function
-- (from migration 20251215014251_fix_deduct_inventory_transaction_type.sql)
-- ============================================================================
...
'Deducts inventory when orders are inserted. Handles both online (/) and POS (+) separator formats for accessory variants. Logs to stock_transactions with transaction_type.';
