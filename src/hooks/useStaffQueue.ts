...
const STAFF_QUEUE_REFETCH_INTERVAL = 30_000;
 ...
 */
function mapStageToStatus(stage: string): StaffQueueItem['status'] {
  switch (stage) {
    case 'Filling':
    case 'Covering':
    case 'Decorating':
 ...
 */
function mapOrderToQueueItem(order: any): StaffQueueItem {
...
export function useInvalidateStaffQueue() {
  ...
  return async (userId?: string) => {
    if (userId) {
      await queryClient.invalidateQueries({ queryKey: ['staffQueue', userId] });
    } else {
      await queryClient.invalidateQueries({ queryKey: ['staffQueue'] });
    }
  };
}
