...
// Gate mock supervisor login behind environment flag (default OFF)
const DEMO = import.meta.env.VITE_SUPERVISOR_DEMO_LOGIN === "true";

interface SupervisorSignInPageProps {
  onSignIn: (email: string, pin: string) => void;
}

export function SupervisorSignInPage({ onSignIn }: SupervisorSignInPageProps) {
  ...
  const handleSignIn = async () => {
    ...
    setTimeout(() => {
      ...
      if (email.toLowerCase() === "pending@example.com") {
        setError("Your account is pending approval. Please contact your manager.");
        setIsLoading(false);
        return;
      }
  ...
  const handleScanBadge = () => {
    toast.info("Scanning supervisor badge...");
    // Simulate badge scan
    setTimeout(() => {
      setEmail("supervisor@bakery.com");
      setPin("123456");
        ...
        <div className="space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Enter your supervisor credentials to access your workspace
            </p>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email">Email / Supervisor ID</Label>
              <Input
                id="email"
                type="text"
                placeholder="Enter your email or supervisor ID"
                value={email}
